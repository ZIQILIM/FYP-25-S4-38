...
function InboxPage() {
  ...
  const fetchMessages = async () => {
    try {
      const x = await authFetch(
        `${process.env.REACT_APP_API_URL}/messages/getAllYourMsgs`,
        { method: "GET" },
        user,
      );
      if (x.success) {
  ...
  const sendMessageToDB = async () => {
    try {
      await authFetch(
        ...
        { method: "POST", body: JSON.stringify(msgData) },
        user,
      );
    } catch (error) {
      console.error(error);
  ...
  const getFilteredMessages = () => {
    ...
    if (activeTab === "all") {
      ...
      return allMessages.filter(
        (msg) => msg.sender_user_id !== "SYSTEM_ANNOUNCEMENT",
      );
    ...
    <div className="inbox-page">
          ...
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-content">
              <div className="message-detail">
                ...
                <div className="message-body">{selectedMessage.text}</div>
              </div>
            </div>
      ...
      {isModalOpen && activeModal === "selectReciver" && (
        <div
          ...
          onClick={() => setActiveModal("writeMessage")}
        >
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
