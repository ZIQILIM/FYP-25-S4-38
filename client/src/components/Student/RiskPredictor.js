    ...
    const fetchAnalysis = async () => {
      ...
      try {
        ...
        const response = await authFetch(
          `${process.env.REACT_APP_API_URL}/analytics/risk`,
          {
            method: "POST",
    ...
    return (
      <div style={{ padding: "20px", color: "#666" }}>
        ...
        <p style={{ fontSize: "0.8rem" }}>
          Complete this quiz to generate an AI risk analysis.
        </p>
      </div>
    );
  }
    ...
    >
      ...
      {type !== "test" && type !== "weighted" ? (
        <div style={{ width: "100%", height: 180, marginBottom: "20px" }}>
          <h4
            style={{ margin: "0 0 10px 0", fontSize: "0.85rem", color: "#888" }}
          >
            SCORE HISTORY
          </h4>
        ...
        {analysis.history?.length > 1 &&
          ...
          type !== "weighted" && (
            ...
            >
              ...
              <span
                style={{
                  color: analysis.trendValue >= 0 ? "#52c41a" : "#ff4d4f",
                }}
              >
                {analysis.trendValue >= 0 ? "📈 +" : "📉 "}
                {analysis.trendValue} pts
              </span>
            </div>
          )}
